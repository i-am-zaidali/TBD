...
class Interpreter:
    ...
    def at_end(self):
        return self.index > (len(self.ast.body) - 1) or getattr(self.current(), "type", None) is TokenType.EOF
    ...
    def _eval_binop(self, node: BinaryExp, scope: Scope = None):
        ...
        if not left.is_arithmetic_compatible(right):
            raise InterpreterException("Incompatible types for arithmetic operation: {} and {}".format(left, right))
    ...
    def _eval_vardec(self, node: VarDec, constant: bool = False, scope: Scope = None):
        ...
        try:
            ...
            raise InterpreterException(f"Variables cannot be redeclared with the let/const keyword!")
    ...
    def _eval_assignment(self, node: AssignmentExp, scope=None):
        ...
        try:
            ...
            if scope.is_constant(name):
                raise InterpreterException(f"Variable {node.name} is a constant and cannot be reassigned!")
            
        except InterpreterException:
            raise InterpreterException(f"Variable {node.name} needs to be defined first with the let/const keyword!")
